...
class StimCodeCircuit(CodeCircuit):
    """
        Prepares a code circuit class based on the supplied stim circuit
    """
    def __init__(self, stim_circuit: StimCircuit, barriers: bool = True, detector_qubit_time_coords: Union[str,List[List]] = 'no_coords'):
        """
            Args:
                ...
                detector_qubit_time_coords (optional): 
                    1) no_coords (default): no qubit list, time is set to zero
                    2) stim_coords: use the stim detectror args as [qubit0,qubit1,...,qubitN,time]
                    3) [[qubit0,qubit1,...,qubitN,time],...]
                    note: providing an empty list, i.e., [[]] is equivalent to the stim_coords option
        """
        super().__init__()
                ...
                for instruction in stim_circuit:
                    if isinstance(instruction, CircuitRepeatBlock):
                        ...
                        if inst_name == 'QUBIT_COORDS':
                            ...
                            """c_ifs are to be handled, apparently one can also condition gates on sweeps?
                            ...
                            for t1,t2 in zip(stim_targets[::2],stim_targets[1::2]):
                                if t1.is_qubit_target:
                                    ...
                                    self.qc.append(single_qubit_gate_dict[inst_name],qargs=[q2]).c_if(c1,1)

                        elif inst_name == 'M':
                            qubits = [target.value for target in instruction.targets_copy()]
                            invert_result = [target.is_inverted_result_target for target in instruction.targets_copy()]
                            if reps>1:
                                cl_reg_name = 'rep_block_'+str(rep_block_count)+'_rep_'+str(rep_ind)+'_meas_block_'+str(meas_count)
                            else:
                                cl_reg_name = 'block_'+str(rep_block_count)+'_meas_block_'+str(meas_count)
                            ...
                            for i,q in enumerate(qubits):
                                self.measurement_data.append((cl_reg_name,i,q))
                            if flip_qubits!=[]:
                                self.qc.x(flip_qubits)
                            meas_count+=1
                        elif inst_name == 'R':
                            ...
                            self.detectors.append({'clbits': [self.measurement_data[ind][:2] for ind in meas_targets],
                                                   'time': [],
                                                   'qubits': [],
                            ...
                            meas_targets = [t.value for t in instruction.targets_copy()]
                            self.logicals.append({'clbits':[self.measurement_data[ind][:2] for ind in meas_targets]})
                            self.css_z_logical = [self.measurement_data[ind][2] for ind in meas_targets] # for the UnionFindDecoder
        ...
        clbit_dict = {(clbit._register.name,clbit._index):clind for clind,clbit in enumerate(self.qc.clbits)}
        detector_meas_indices = [[clbit_dict[clbit] for clbit in det['clbits']] for det in self.detectors]
        self.det_ref_values = [sum(noiseless_measurements[meas_list])%2 for meas_list in detector_meas_indices]
            ...
            if isinstance(instruction, CircuitInstruction):
                if instruction.name in decompose_1_dict:
                    ...
                    if instruction.name in ('MXX','MYY','MZZ'):
                        if len(set(target_list)) < len(target_list): # avoiding overlaps that result from stim broadcating
                            target_list = [target_list[i:i+2] for i in range(0,len(target_list),2)]
                        else:
                            target_list = [target_list]
                        for targets in target_list:
                            for gate,qubit_ind in basis_gates:
                                if len(qubit_ind)==2:
                                    qubit_list = [target_pair[q_ind].value 
                                                for target_pair in zip(targets[::2],targets[1::2]) 
                                                for q_ind in qubit_ind]
                                else:
                                    qubit_list = [target_pair[qubit_ind[0]].value for target_pair in zip(targets[::2],targets[1::2])]

                                if gate=='M':
                                    inv_list = [(target_pair[0].is_inverted_result_target + target_pair[1].is_inverted_result_target)%2 
                                                for target_pair in zip(targets[::2],targets[1::2])]
                        ...
                        for gate,_ in basis_gates:
                            qubit_list = [target.value for target in targets]
                            if gate=='M':
                                inv_list = [target.is_inverted_result_target for target in targets]
                                for i,inv in enumerate(inv_list):
                                    if inv:
                                        qubit_list[i] = target_inv(qubit_list[i])
                                decomp_gate.append(gate,qubit_list,arg)
                    ...
                    """
                    warnings.warn("The circuit contains MPAD instructions that are ignored in the conversion. This can affect the measurement outcomes, but not the detectors.")
                    pass
                ...
                decomposed_stim_circuit.append(
                    CircuitRepeatBlock(instruction.repeat_count,
    ...
    def string2nodes(self, string, **kwargs):
        ...
        nodes = string2nodes_with_detectors(string=string,detectors=self.detectors,
                                            logicals=self.logicals,clbits=self.qc.clbits,
                                            det_ref_values=self.det_ref_values,  
                                            **kwargs)
        return nodes
    
    def post_process_result_string(self, string):
        """
        Accounts for the effect of MPAD and inverted measurement outcomes

        MPAD in principle can only flip some detectors/logicals if a bit is 
        included as a measurement outcome. Can we deal with that differently?

        same holds for inverted measurements... they only matter if they enter a detector
        """
